...
func GenerateKey() (SecretKey, PublicKey, error) {
	...
	// generate private key (also call secret)
	for i := 0; i < 2; i++ {
		for j := 0; j < n; j++ {
			block := make([]byte, 32)
			...
			if i == 0 {
				sec.ZeroPre[j] = Block(block)
			} else {
				sec.OnePre[j] = Block(block)
			}
		}
	}
	...
	for i := 0; i < 2; i++ {
		for j := 0; j < n; j++ {
			if i == 0 {
				hdata := sha256.Sum256(sec.ZeroPre[j][:])
				pub.ZeroHash[j] = hdata
			} else {
				hdata := sha256.Sum256(sec.OnePre[j][:])
				pub.OneHash[j] = hdata
			}
		}
	}
